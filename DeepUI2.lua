...
local function StringToColor(color)
    local StringSplitted = string.split(color, ', ');
    return Color3.new(tonumber(StringSplitted[1]), tonumber(StringSplitted[2]), tonumber(StringSplitted[3]))
end;
    ...
    function window:CreateTab(name)
        ...
        function tab:CreateSector(name,side)
            ...
            function sector:AddToggle(text, default, callback, flag)
                ...
                function toggle:AddKeybind(SaveManager)
                    local keybind = { }
					
					if not SaveManager.Data[text..' Key'] then 
						SaveManager.Data[text..' Key'] = 'None';
					end;

					local default = (SaveManager.Data[text..' Key'] ~= 'None' and Enum.KeyCode[SaveManager.Data[text..' Key']]);
					local flag = '';

					-- default, flag,keycallback
                    ...
                    function keybind:Set(key)
						--keycallback(key)
						local newhotkey = tostring(key):split('KeyCode.')[2]
						SaveManager.Set[realToggleText..' Key'] = newhotkey;
                ...
                function toggle:AddColorpicker(SaveManager,callback)
                    local colorpicker = { }
                    -- default, callback, flag
                    local realText = text;
                    local flag = '';
                    local default = Color3.fromRGB(255, 255, 255);

                    if not SaveManager.Data[realText..' Color'] then 
						SaveManager.Data[realText..' Color'] = tostring(default);
					end;

					default = (SaveManager.Data[realText..' Color'] ~= tostring(default) and StringToColor(SaveManager.Data[realText..' Color']));
                    ...
                    function colorpicker:Set(value)
                        ...
                        SaveManager.Set[realText..' Color'] = tostring(color);
